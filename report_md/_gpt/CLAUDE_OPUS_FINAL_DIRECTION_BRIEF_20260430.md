...
### D2 — R11D wording: **SOFT, with conditional upgrade path**

**Ruling.** Use Codex's softer phrasing in current text:

> "Under the tested AIHWKit PCM UnitCell regime, PCM update/noise physics make 4-bit forward-path training viable where the tested pure 4-bit AIHWKit baselines collapse."

**Conditional upgrade.** If PCMPresetDevice (Batch B/C) trains stably (>70%), revise to:

> "Realistic PCM device models (UnitCell and Device) enable 4-bit forward-path training where pure 4-bit quantization collapses; we attribute this to the smoothing of weight updates by PCM pulse-update dynamics."

If PCMPresetDevice collapses (<45% by epoch 50), keep soft phrase and add a limitations sentence: "The viability is preset-dependent; PCMPresetDevice did not train under the matched recipe (see SI §S-PCM-Preset-Sensitivity)."

### D3 — PCMPresetDevice decision: **wait for Batch B/C, then bifurcate**

**Ruling.**

- If `t13v2_r11d_5a_pcm_PCMPresetDevice_seed123` reaches ≥ 70% best-test by epoch 50 → integrate as **material sensitivity** (preset-agnostic claim strengthens).
- If 45–70% → run one diagnostic at lr=5e-3 only; document outcome as preset sensitivity.
- If < 45% by epoch 50 → terminate run, tag UnitCell as **preset-specific**, move PCMPresetDevice to limitations / SI.
- **Do NOT** retrain everything for PCMPresetDevice. Time cost vs reviewer credibility is unfavorable.

### D4 — Progressive quantization priority: **6-bit pilot first**

**Ruling.** Agree with Codex. Order after Batch B/C:

1. 6-bit pilot (seed123 only) — closes the precision-Pareto question between 4-bit (drifty) and 8-bit (flat).
2. If 6-bit shows interesting Pareto point (drift between 4-bit and 8-bit, fresh within 0.5 pp of 8-bit), run seeds 456/789.
3. Progressive quantization (Gemini's request) is **deferred to revision response**, not pre-submission. Reasoning: it's a method extension, not a missing baseline.

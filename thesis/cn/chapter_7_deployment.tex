...
为检验该排序是否能转化为部署保护策略，本研究追加了一个 P1 上界型保护 pilot：
在 $2048$ 张 CIFAR-10 子集与 $3$ 个种子上，将所有模拟层重采样为 fresh D2D 实例，
然后把 full42 排名前 $K$ 的敏感层保留在 checkpoint D2D（\texttt{freeze\_topK\_d2d}），其余层仍使用 fresh D2D。
结果如图~\ref{fig:protection-map-p1} 所示。
完全 fresh 的全模拟模型仅为 $8.89\%$；保护前 $10$ 层提高到 $26.22\pm3.62\%$，保护前 $13$ 层提高到 $39.34\pm6.16\%$。
这说明 full42 排序确实携带救援信号，但只保护少数 outlier 仍不足以恢复接近 checkpoint 的 $92.58\%$ 源域子集精度。
另外，简单把 top-$10/13$ 层设为无噪声高精度上界并不能救回 fresh 全层失配，提示失配补偿不是局部旁路即可解决的线性叠加问题。
因此，混合精度策略在本文中只能作为候选设计方向；最终部署级证据仍需要 full-test 或 fresh-instance 协议验证，并可能需要与 Ensemble HAT 训练期重采样联合，而非仅做推理期层保护。

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\textwidth]{../figures/mixed_precision/fig_protection_map_pilot_p1_20260510}
    \caption{基于 full42 排序的 P1 保护策略 pilot。\texttt{freeze\_topK\_d2d} 将排名前 $K$ 的敏感层保留为 checkpoint D2D，其余层重采样为 fresh D2D；\texttt{noiseless\_topK\_hp\_upper} 为乐观高精度/无噪声上界。结果显示前 $13$ 层保护可产生明显救援，但仍远低于源域 checkpoint 精度。}
    \label{fig:protection-map-p1}
\end{figure}

...
def _resolve_pcm_preset(preferred=None):
    ...
    last_err = None
    if preferred:
        # STRICT mode: only try presets matching the requested name
        # If none succeed, raise RuntimeError immediately (no fallback)
        ...
        if not candidates:
            raise RuntimeError(
                ...
                f"Available: {[c for _, c in PCM_PRESET_REGISTRY]}."
            )
        ...
        raise RuntimeError(
            ...
            f"Tried: {[f'{m}.{c}' for m, c in candidates]}. "
            f"Last error: {last_err}"
        )
    # Default mode: try registry in order (original behavior)
...
def make_rpu_config(inp_res=1.0 / 256.0, out_res=1.0 / 256.0, modifier_std_dev=0.10, pcm_preset=None):
    pcm_device, pcm_name = _resolve_pcm_preset(preferred=pcm_preset)
...
def main():
    ...
    parser.add_argument("--pcm-preset", type=str, default=None, help="Force specific PCM preset (PCMPresetUnitCell or PCMPresetDevice)")
    ...
    if getattr(args, "resume", ""):
        ...
        with torch.serialization.safe_globals([InferenceRPUConfig]):

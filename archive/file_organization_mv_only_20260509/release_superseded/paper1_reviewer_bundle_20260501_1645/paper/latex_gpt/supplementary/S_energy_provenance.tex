...
This note documents the origin of every constant used in the first-order analytical energy profiler (\S 3.7 of the main text).  The purpose is to allow reviewers to distinguish literature-anchored numbers from order-of-magnitude engineering estimates.
...
\bottomrule
\end{tabular}
\caption{Provenance of energy-model constants. ``Literature proxy'' means the value is taken from a published silicon or thin-film measurement but is used as a generic placeholder for the target device technology. ``Order-of-magnitude estimate'' means the value is a rounded central tendency drawn from a literature range or an analytical surrogate, intended for relative comparison rather than routed-circuit prediction.}

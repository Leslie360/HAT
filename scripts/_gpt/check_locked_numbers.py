...
def main() -> int:
    locked_numbers: list[LockedNumber] = [
        ...
        LockedNumber(
            id="H4",
            description="Ensemble HAT single-checkpoint fresh-instance mean",
        ...
        LockedNumber(
            id="H4_R10A",
            description="Ensemble HAT three-seed fresh-instance headline",
            json_path=REPO_ROOT / "report_md" / "_gpt" / "json_gpt" / "r10a_canonical_ensemble_hat_3seed_fresh_eval.json",
            accessor=lambda d: d["cross_seed"]["mean_of_seed_means"],
            expected=86.16,
            tolerance=0.01,
        ),
        LockedNumber(
            ...
            accessor=lambda d: d["rows"][0]["fresh_mean"],
            expected=83.03,
            tolerance=0.05,
        ),
        LockedNumber(
            id="H10_R10D_NL15",
            description="R10D NL=1.5 fresh-instance mean",
            json_path=REPO_ROOT / "report_md" / "_gpt" / "json_gpt" / "r10d_nl_interpolation_summary.json",
            accessor=lambda d: d["rows"][1]["fresh_mean"],
            expected=82.63,
            tolerance=0.05,
        ),
        LockedNumber(
            id="H11_R10D_NL18",
            description="R10D NL=1.8 fresh-instance mean",
            json_path=REPO_ROOT / "report_md" / "_gpt" / "json_gpt" / "r10d_nl_interpolation_summary.json",
            accessor=lambda d: d["rows"][2]["fresh_mean"],
            expected=80.31,
            tolerance=0.05,
        ),
        LockedNumber(
        ...
        LockedNumber(
            ...
            json_path=REPO_ROOT / "report_md" / "_gpt" / "json_gpt" / "crosssim_standard_noise.json",

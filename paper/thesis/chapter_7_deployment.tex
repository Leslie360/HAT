...
\textbf{MLP-only linearization under severe NL.}  The MLP-only rescue checkpoint achieves $87.79\,\%$ source-domain accuracy under $NL=2.0$, which is competitive with the canonical V4 baseline.  However, its fresh-instance accuracy is $32.12\pm7.72\,\%$, and the all-linear upper bound (best $87.49\,\%$, final $84.81\,\%$) reaches only $32.60\pm9.18\,\%$ on fresh instances.  These numbers do not preserve the ranking in any useful sense: although $32\,\%$ is strictly greater than the $10\,\%$ fixed-mask collapse, it is below the threshold of commercial viability for most vision tasks.  The orange-zone classification reflects the fact that the ranking is technically preserved ($32 > 10$) but \emph{operationally collapsed} because neither protocol is deployable.
...
\subsection{Severe nonlinearity: the $32\,\%$ fresh-instance ceiling}

The third collapse boundary is severe write nonlinearity ($NL=2.0$).  On the source-domain training-time array, the unmitigated $NL=2.0$ baseline reaches $27.72\pm0.82\,\%$, while MLP-only linearization recovers to $87.79\,\%$ and all-linear control reaches $87.49\,\%$.  These source-domain numbers suggest that the MLP path is the dominant recoverable bottleneck.

On fresh instances, however, the picture inverts.  The MLP-only checkpoint achieves only $32.12\pm7.72\,\%$ on fresh instances, and the all-linear bound (best $87.49\,\%$, final $84.81\,\%$) reaches only $32.60\pm9.18\,\%$.  The large cross-instance standard deviations ($7.72$~pp and $9.18$~pp) indicate that some fresh arrays are tolerable while others are catastrophic---a bimodal-like spread that is qualitatively different from the tight $1.54$~pp spread of canonical Ensemble HAT.  The ranking $32\,\% > 10\,\%$ is technically preserved, but operationally the severe-NL regime is a collapse zone because $32\,\%$ accuracy is below the threshold of utility for most classification deployments.

Extrapolating from the $NL=2.0$ data, one can bound the severity at which the MLP-only rescue becomes viable.  If $NL=1.0$ yields $86.37\,\%$ fresh-instance accuracy and $NL=2.0$ yields $32\,\%$, the relationship is strongly nonlinear.  A linear interpolation would predict a $59\,\%$ crossing at $NL=1.5$, but there is no evidence for linearity; the gradient-scaling surrogate becomes increasingly ill-conditioned as $NL$ grows, and the dual-attention collapse (Section~\ref{sec:failure-dual-attention-nl}) suggests a bifurcation rather than a gradual ramp.  The conservative bound is that \emph{any $NL \ge 2.0$ is in the collapse zone for fresh-instance ViT deployment}, and the threshold may be as low as $NL \approx 1.5$--$1.7$.

...
def test_second_order_branch_matches_first_order():
    ...
    for i, line in enumerate(lines):
        if (
            'correction = alpha * torch.where(grad_output >= 0' in line
            or 'correction = alpha * torch.where(grad_output_fp32 >= 0' in line
        ):
...
def test_nl_1p5_no_gradient_explosion():
    """Regression test: 1 < NL < 2 must not produce inf/nan or huge gradients."""
    x = torch.tensor([1.0 - 1e-9, 1e-9], requires_grad=True)
    q = StraightThroughQuantize.apply(x, 16, 0.0, 1.0, 1.5, -1.5, None, True, 0.15, 1.0)
    ...
    assert torch.isfinite(grad_input).all(), f"NL=1.5 produced non-finite grad: {grad_input}"
    assert grad_input.abs().max().item() < 5.0, f"NL=1.5 gradient still exploded: {grad_input}"
    print("✅ test_nl_1p5_no_gradient_explosion passed")

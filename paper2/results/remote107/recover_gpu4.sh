#!/bin/bash
# Recovery script for GPU4 — idempotent, resilient to single-job failures
set -uo pipefail
...
GPU=4
LOG="robustness_gpu4_recovery_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT/logs"

# Idempotent eval: skip if output JSON already exists
run_if_missing() {
...
for s_d2d in 0.06 0.10; do run_if_missing "$CKPT/p28b_fixed500_seed42" true 0.01 "$s_d2d" 256 3373 "p28b_d2d_${s_d2d}"; done
for s_c2c in 0.02 0.05 0.10; do run_if_missing "$CKPT/p28b_fixed500_seed42" true "$s_c2c" 0.02 256 3373 "p28b_mis_c2c_${s_c2c}"; done
for s_d2d in 0.04 0.06 0.10; do run_if_missing "$CKPT/p28b_fixed500_seed42" true 0.01 "$s_d2d" 256 3373 "p28b_mis_d2d_${s_d2d}"; done
for n_st in 128 256; do run_if_missing "$CKPT/p28b_fixed500_seed42" true 0.01 0.02 "$n_st" 3373 "p28b_nstates_${n_st}"; done

echo "[$(date +%H:%M:%S)] GPU4 recovery batch complete."

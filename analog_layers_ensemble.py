...
def _retention_decay_factor(cfg: AnalogLinearConfig) -> float:
    ...
    return (
        A_1 * math.exp(-t / cfg.tau_1)
        + A_2 * math.exp(-t / cfg.tau_2)
        + A_0
    )
...
class AnalogConv2d(nn.Module):
    ...
    def _apply_retention(self, G_pos, G_neg):
        ...
        G_pos_d = cfg.G_min + (G_pos - cfg.G_min) * decay
        G_neg_d = cfg.G_min + (G_neg - cfg.G_min) * decay

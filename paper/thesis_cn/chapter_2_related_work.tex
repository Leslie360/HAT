...
DNN+NeuroSim 是这一样板的奠基工作，其支持完整的片上训练，并已与 fabricated macro 进行验证~\citep{peng2020dnnneurosim}。
...
MemTorch 中的频率讨论聚焦于推理时故障注入（inference-time fault injection），而非训练时 D2D 重采样。
...
而注意力分数、softmax 归一化及其他动态交互因精度要求和 irregular 访问模式而保留数字域或委托给专用混合信号子路径~\citep{kim2025hemlet,lin2024hardsea}。

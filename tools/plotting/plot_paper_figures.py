...
COL = {
    ...
    "muted": "#666666",
    "grid": "#E1E6EB",
    "blue": "#264653",     # NATURE_NAVY
    "orange": "#E76F51",   # NATURE_CORAL
    "green": "#2A9D8F",    # NATURE_TEAL
    "gold": "#E9C46A",     # NATURE_GOLD
    "gray": "#AAAAAA",     # GRAY_LIGHT
}
...
PROXY_BLUE_CMAP = LinearSegmentedColormap.from_list(
    "proxy_blue_academic",
    ["#F8FAFC", "#E3EDF5", "#B7D1E4", "#6FA4C9", "#2F6690"],
)
...
def plot_fig3_snr_curves(output_dir: Path):
    ...
    palette = [COL["blue"], COL["green"], COL["orange"], COL["gold"]]
    for idx, gamma in enumerate(gamma_values):
        p_in = np.power(x, 1.0 / gamma)
        snr = x / np.sqrt(np.maximum(p_in, 1e-12))
        ax.plot(x, snr, linewidth=2.1, color=palette[idx % len(palette)], label=rf"$\gamma={gamma}$")

    ax.set_xscale("log")
    ax.set_xlabel("Normalized pixel intensity")
    ax.set_ylabel("Normalized SNR (a.u.)")
    ax.set_title("SNR under inverse-gamma compensation", loc="left", pad=4)
    ax.legend(
    ...
    save_figure_pair(fig, output_dir, "fig3_snr_curves")
    plt.close(fig)
...
def plot_fig6_physical_compensation(output_dir: Path):
    ...
    fig, axes = plt.subplots(
        ...
        2,
        figsize=(9.4, 3.75),
        gridspec_kw={"width_ratios": [1.05, 0.95], "wspace": 0.30},
    )

    im = axes[0].imshow(heatmap, cmap=SOFT_TEAL_CMAP, aspect="auto")
    ...
    axes[0].set_title("A  Compensation gain", loc="left", pad=4)
    finite_vals = heatmap[np.isfinite(heatmap)]
    threshold = float(np.nanmedian(finite_vals)) if finite_vals.size else 0.0
    for i in range(heatmap.shape[0]):
        for j in range(heatmap.shape[1]):
            val = heatmap[i, j]
            text_color = "white" if np.isfinite(val) and val > threshold else COL["ink"]
            axes[0].text(j, i, f"{val:.2f}", ha="center", va="center", fontsize=8.7, color=text_color)
    fig.colorbar(im, ax=axes[0], fraction=0.046, pad=0.025)

    x = np.arange(2)
    width = 0.34
    standard_vals = [group3["std_clean"], std_mean]
    hat_vals = [group3["hat_clean"], hat_mean]
    axes[1].bar(
        x - width / 2,
        standard_vals,
        width=width,
    ...
    for xpos, val in zip([*list(x - width / 2), *list(x + width / 2)], [*standard_vals, *hat_vals]):
        if val > 88:
            y_pos, va, color = val - 5.5, "top", "white"
        else:
            y_pos, va, color = val + 1.5, "bottom", COL["ink"]
        axes[1].text(xpos, y_pos, f"{val:.1f}", ha="center", va=va, fontsize=8.5, color=color)
    axes[1].set_xticks(x)
    axes[1].set_xticklabels(["Clean acc.", "CIFAR-10-C\nmCE"])
    axes[1].set_ylim(0, 100)
    axes[1].set_ylabel("Metric value (%)")
    axes[1].set_title("B  Robustness summary", loc="left", pad=4)
    axes[1].legend(
        loc="upper center",
    ...
    enable_major_y_grid(axes[1])
    fig.tight_layout(pad=0.45)
    save_figure_pair(fig, output_dir, "fig6_physical_compensation")
    plt.close(fig)
...
def plot_fig9_noise_sensitivity(output_dir: Path):
    ...
    fig_width = 9.6 if n_heatmaps == 1 else 11.4
    width_ratios = [1.16] * n_heatmaps + [1.0]
    fig, axes = plt.subplots(
        1,
        n_heatmaps + 1,
        figsize=(fig_width, 3.75),
        sharey=False,
        gridspec_kw={"width_ratios": width_ratios, "wspace": 0.48},
    )
    ...
    heatmap_values = []
    for _, _, model_rows in heatmap_specs:
        for row in model_rows:
            acc = row.get("mean_acc", row.get("test_acc_mean", row.get("acc_mean")))
            if acc is not None:
                heatmap_values.append(float(acc))
    if heatmap_values:
        low = float(np.nanmin(heatmap_values))
        high = float(np.nanmax(heatmap_values))
        margin = max(0.18, (high - low) * 0.12)
        vmin = max(0.0, low - margin)
        vmax = min(100.0, high + margin)
    else:
        vmin = 0.0
        vmax = 100.0
    ...
    for ax, (model_name, title, model_rows) in zip(axes[:n_heatmaps], heatmap_specs):
        ...
        ax.set_title(f"(a) {title} noise grid", loc="left", pad=7)
        if ax is axes[0]:
        ...
        for i in range(grid.shape[0]):
            for j in range(grid.shape[1]):
                ...
                ax.text(j, i, f"{grid[i, j]:.1f}", ha="center", va="center", fontsize=8.4, color=txt_color, fontweight="bold")

    if heatmap_im is not None and heatmap_axes_with_data:
        cbar = fig.colorbar(heatmap_im, ax=heatmap_axes_with_data, fraction=0.035, pad=0.035)
    ...
    if adc_rows:
        ...
        palette = {"convnext": COL["blue"], "tinyvit": COL["green"]}
        ...
        adc_ax.set_title("(b) ADC precision", loc="left", pad=7)
        adc_ax.axvline(order.index("6-bit"), color="#666666", linestyle="--", linewidth=1.0)
        adc_ax.text(order.index("6-bit") + 0.08, 9, "6-bit knee", rotation=90, va="bottom", ha="left", fontsize=8.3, color="#666666")
    ...
    if not any_heatmap and not adc_rows:
        ...
        return
    fig.tight_layout(pad=0.50, w_pad=2.2)
...
def plot_fig_fresh_instance_ablation(output_dir: Path):
    ...
    ax = axes[0]
    categories = [
        ("Standard HAT", standard_instances, "#CC6677"),
        ("Ensemble HAT", ensemble_instances, COL["green"]),
    ]
    ...
    colors = ["#B9C0CA", "#A8BBCD", "#86A5BF", COL["green"], "#D6AD60"]
    bars = ax.bar(
...
def plot_fig_sobol_sensitivity(output_dir: Path):
    ...
    colors = [COL["blue"], COL["green"], "#B8BDC6"]

    fig, ax = plt.subplots(figsize=(5.55, 3.10))
    ...
    ax.set_axisbelow(True)
    ax.legend(loc="upper center", ncol=3, frameon=True, bbox_to_anchor=(0.5, 1.18))
...
def plot_fig_corr_d2d(output_dir: Path):
    ...
    for xpos, mean_val, std_val, color, delta in zip(x, means, stds, colors, deltas):
        ...
        ax.text(
            xpos,
            min(mean_val + std_val + 0.45, 89.6),
            label,
            ha="center",
            va="bottom",
            fontsize=8.4,
    ...
    ax.set_title("Correlated-D2D stress test", loc="left", pad=3)
    enable_major_y_grid(ax)
    fig.tight_layout(pad=0.45)

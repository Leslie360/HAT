...
\texttt{freeze\_top42\_d2d}（完整测试集，$10$ 个未见 D2D 实例，每实例 $3$ 次 C2C 蒙特卡洛前向传播）。
如图~\ref{fig:fresh-instance-protection-sharded} 所示，全新失配配置仍稳定在 $10.00\pm0.01\%$，
保护前 $20$ 层恢复到 $67.65\pm3.21\%$，保护前 $30$ 层达到 $86.04\pm0.95\%$，
而全 $42$ 层保留检查点 D2D 为 $91.66\pm0.19\%$。
该结果说明完整测试集保护曲线并非少数 D2D 实例的偶然现象：top-$30$ 保护在 $10$ 个未见实例上保持稳定救援，
但它仍是推理期保护诊断，不等同于已经实现的混合精度硬件方案。
...
\begin{figure}[t]
    ...
    \caption{基于 full42 排序的分片式 fresh-instance 保护复核。每个策略在完整 CIFAR-10 测试集上评估 $10$ 个未见 D2D 实例、每实例 $3$ 次 C2C 蒙特卡洛前向传播。top-$30$ 保护在多实例上达到 $86.04\pm0.95\%$，显著高于全新失配配置的 $10.00\pm0.01\%$，但该图仍表示推理期保护诊断，而非已实现的混合精度硬件方案。}

...
def main() -> None:
    ...
    ckpt_root = REPO_ROOT / "checkpoints" / "_gpt" / "cx_k3_dgeff"
    ...
    for delta in DELTA_VALUES:
        ...
        train_log = REPO_ROOT / "logs" / "_gpt" / f"cx_k3_train_{tag}.log"
    ...
    out_json = json_dir / "cx_k3_dgeff_continuation.json"
    out_md = report_dir / "CODEX_CX_K3_CONTINUATION_20260421.md"
    ...
    for tag, row in aggregate["results"].items():
        lines.append(
            f"| {tag} | {row['delta_g_eff']:.2f} | {row['train_best_acc'] if row['train_best_acc'] is not None else float('nan'):.2f} | {row['train_best_epoch']} | {row['fresh_mean']:.2f} | {row['fresh_std']:.2f} |"
        )
    lines.extend([
        "",
    ...
    out_md.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"K3 continuation complete. Summary written to {out_json}")

...
def plot_rows(rows: List[dict], figure_path: str) -> None:
    ...
    width = 0.36
    colors = {"full": "#2F5D76", "affected": "#D07A5D"}
    fig, axes = plt.subplots(1, 2, figsize=(10.2, 3.85), sharex=True)

    panels = [
        (axes[0], full_cos, affected_cos, "A  Direction cosine", "Cosine vs. NL=1 baseline", (0.0, 1.04)),
        (axes[1], full_norm, affected_norm, "B  Gradient norm ratio", "Norm ratio vs. NL=1 baseline", (0.0, 1.18)),
    ]
    ...
    fig.tight_layout(w_pad=1.35, pad=0.45)

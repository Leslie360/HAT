#!/bin/bash
# Weekend Pipeline — Auto-run experiments on GPUs 4/5/6/7
# Usage: nohup bash weekend_pipeline.sh > weekend_pipeline.log 2>&1 &
# Wait for current lm-eval to finish, then launch next experiments.
...
wait_lm_eval() {
    ...
    while pgrep -f "p3_hat_lm_eval" > /dev/null; do
        sleep 60
    done
    echo "[$(date)] GPU $GPU: lm-eval done, starting pipeline."
}

# ── GPU 4: 2.8B mismatch + n_states ──────────────────────────
run_gpu4() {
    wait_lm_eval 4

    # 2.8B sigma_c2c mismatch
...
# ── GPU 5: 2.8B last2/last4 + D2D seed ──────────────────────
run_gpu5() {
    wait_lm_eval 5

    # 2.8B last2 clean eval
...
# ── GPU 6: 6.9B mismatch (c2c) + n_states ───────────────────
run_gpu6() {
    wait_lm_eval 6

    # 6.9B sigma_c2c mismatch
    for sigma in 0.0 0.01 0.02 0.05 0.10; do
...
# ── GPU 7: 6.9B mismatch (d2d) + D2D seed ────────────────────
run_gpu7() {
    wait_lm_eval 7

    # 6.9B sigma_d2d mismatch
    for sigma in 0.0 0.01 0.02 0.05 0.10; do
...
echo "[$(date)] Weekend pipeline launched. 4 background jobs started."
echo "Monitor logs: weekend_gpu{4,5,6,7}.log"

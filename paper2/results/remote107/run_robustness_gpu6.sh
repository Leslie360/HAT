...
GPU=6
LOG="robustness_gpu6_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT/logs"
...
for s_c2c in 0.02 0.05 0.10; do eval_job "$CKPT/p69b_fixed500_seed42" true "$s_c2c" 0.02 256 3373 "p69b_c2c_${s_c2c}"; done
for s_d2d in 0.0 0.02 0.04 0.06 0.10; do eval_job "$CKPT/p69b_fixed500_seed42" true 0.01 "$s_d2d" 256 3373 "p69b_d2d_${s_d2d}"; done
eval_job "$CKPT/p69b_fixed500_seed42" true 0.01 0.02 256 3373 "p69b_mis_c2c_0.02"
for n_st in 256 512; do eval_job "$CKPT/p69b_fixed500_seed42" true 0.01 0.02 "$n_st" 3373 "p69b_nstates_${n_st}"; done

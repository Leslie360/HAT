...
LOG="/home/qiaosir/projects/compute_vit/paper2_aihwkit_baseline/logs/r11d_5a_pcm_$(date +%Y%m%d_%H%M%S).log"
mkdir -p "$(dirname "$LOG")"
echo "=== R11D-5a PCM (lr=1e-3) ===" | tee -a "$LOG"
nohup "$PYTHON" "$SCRIPT" \
    --run-id r11d_5a_pcm \
    --epochs 100 \
    --batch-size 64 \
    ...
    --device cuda \
    --workers 0 \
    --save-dir paper2_aihwkit_baseline/checkpoints/r11d_5a_pcm \
    --log-interval 1 \

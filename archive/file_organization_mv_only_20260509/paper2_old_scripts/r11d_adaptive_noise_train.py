#!/usr/bin/env python3
"""
R11D-AdaptiveNoise: PCM training with annealed modifier noise.
Based on r11d4_train_pcm.py.

Anneals modifier_std_dev from a high initial value down to a low final value
using cosine annealing, synchronized with the LR schedule. Goal: reduce
seed sensitivity by providing stronger regularization early, then stabilizing.
"""
...
def set_modifier_std_dev(model, std_dev):
    """Dynamically update modifier std_dev on all analog tiles."""
    updated = 0
    for module in model.modules():
        if isinstance(module, AnalogLinear):
            for tile in module.analog_tiles():
                tile.rpu_config.modifier.std_dev = std_dev
                updated += 1
    return updated


def get_cosine_noise(epoch, epochs, noise_start, noise_end):
    """Cosine annealing for noise, synced with LR schedule."""
    if epochs <= 1:
        return noise_end
    progress = epoch / (epochs - 1)
    return noise_end + (noise_start - noise_end) * 0.5 * (1.0 + np.cos(np.pi * progress))
...
def main():
    ...
    parser.add_argument("--save-dir", default="paper2_aihwkit_baseline/checkpoints/r11d_adaptive_noise")
    ...
    parser.add_argument("--modifier-std-dev", type=float, default=0.10,
                        help="Fixed modifier std dev (used when adaptive-noise is False)")
    parser.add_argument("--adaptive-noise", action="store_true",
                        help="Enable cosine-annealed modifier noise schedule")
    parser.add_argument("--noise-start", type=float, default=0.15,
                        help="Initial modifier std dev for adaptive noise")
    parser.add_argument("--noise-end", type=float, default=0.02,
                        help="Final modifier std dev for adaptive noise")
    parser.add_argument("--early-stop-patience", type=int, default=0)
    parser.add_argument("--early-stop-min-delta", type=float, default=0.0)
    parser.add_argument("--run-id", default="r11d_adaptive_noise")
    ...
    print(f"=== R11D-AdaptiveNoise AIHWKit PCM Device Model ===")
    print(f"Device: {device}")
    print(f"PyTorch: {torch.__version__}")
    print(
        "Config: "
        f"run_id={args.run_id}, epochs={args.epochs}, bs={args.batch_size}, lr={args.lr}, "
        f"adaptive_noise={args.adaptive_noise}, noise_start={args.noise_start}, noise_end={args.noise_end}"
    )
    ...
    init_std = args.noise_start if args.adaptive_noise else args.modifier_std_dev
    try:
        model, rpu_config, pcm_name = build_model(
            num_classes=num_classes,
            inp_res=args.inp_res,
            out_res=args.out_res,
            modifier_std_dev=init_std,
        )
    ...
    provenance = {
        ...
        "noise_schedule": "cosine" if args.adaptive_noise else "fixed",
    }
    ...
    for epoch in range(args.epochs):
        ...
        if args.adaptive_noise:
            current_std = get_cosine_noise(epoch, args.epochs, args.noise_start, args.noise_end)
            n_updated = set_modifier_std_dev(model, current_std)
        else:
            current_std = args.modifier_std_dev
            n_updated = 0
        ...
        history.append({
            ...
            "modifier_std_dev": round(current_std, 4),
        ...
        if (epoch + 1) % args.log_interval == 0 or epoch == 0 or epoch == args.epochs - 1:
            ...
            noise_tag = f" noise={current_std:.3f}({n_updated})" if args.adaptive_noise else ""
            print(f"Epoch {epoch+1:3d}/{args.epochs} | Train {train_acc:.2f}% | Test {test_acc:.2f}% | Best {best_acc:.2f}% | {epoch_time:.1f}s/epoch | ETA {eta/3600:.1f}h{noise_tag}")

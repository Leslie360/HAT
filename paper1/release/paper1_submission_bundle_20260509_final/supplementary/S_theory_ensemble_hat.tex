...
This note provides a formal mathematical derivation of Ensemble Hardware-Aware Training (HAT) and its relationship to classical implicit regularization. We aim to show why the per-epoch resampling of device mismatch patterns leads to a more robust representation compared to the fixed-mask approach typically used in standard HAT. By expanding the loss function around the nominal weights, we reveal how the ensemble objective effectively punishes the curvature of the loss surface specifically in the directions of device-to-device variation.

\subsection*{S.1 Setup and notation}

To establish the framework, we consider an analog crossbar array with differential-pair conductance mapping. Let $\theta \in \mathbb{R}^d$ denote the digital weights (network parameters) to be programmed onto the array, and let $M \in \mathbb{R}^d$ denote the per-device fractional device-to-device (D2D) mismatch map. The programmed conductance pair for weight $\theta_i$ is perturbed multiplicatively:
\begin{equation}
...
\paragraph{Standard HAT (fixed-mask).} In the standard approach, one mismatch map $M_0 \sim \mathcal{D}_{\text{D2D}}$ is sampled at initialization and held fixed for the entire training duration:
\begin{equation}
...
\paragraph{Ensemble HAT (distribution-matching).} In our proposed ensemble method, the mismatch map is re-sampled at the start of every training epoch, yielding a sequence $\{M^{(e)}\}_{e=1}^{E}$ with $M^{(e)} \stackrel{\text{i.i.d.}}{\sim} \mathcal{D}_{\text{D2D}}$. The objective is:
\begin{equation}

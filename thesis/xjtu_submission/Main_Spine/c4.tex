...
数据集按难度递进排列：
CIFAR-10（10 类，$32\times32$）$\rightarrow$ CIFAR-100（100 类，$32\times32$）$\rightarrow$
Flowers-102（102 类，数据稀缺）$\rightarrow$ ImageNet-1k（1{,}000 类，$224\times224$）。
...
但标准噪声部署（R3）与均匀噪声 HAT（R4）均崩溃至约 $1.00\%$；对应的 ADC 扫描记录显示 CIFAR-100 R4 在所有 ADC 配置下均保持 $1.00\%$。
这一极端退化说明：ResNet-18 的浅层特征提取器在 100 类细粒度任务上对模拟噪声的容错窗口极窄，
HAT 的常规梯度缩放策略不足以恢复其判别几何。

\subsection{ConvNeXt-Tiny}

ConvNeXt-Tiny 的 CIFAR-10 与 CIFAR-100 结果来自论文源数据汇总文件，汇总于表~\ref{tab:convnext-results}。
...
Tiny-ViT 的完整 baseline 矩阵来自论文源数据汇总文件，汇总于表~\ref{tab:tinyvit-results}。
...
导致预测类别与真实标签系统性错位。
该 JSON 因此不代表模型真实的 ImageNet-1k 能力，而仅用于当前仿真体制的数据通路一致性检查。
完整的 ImageNet-1k 基准需在修复数据加载管道后重新运行。

\section{Fresh-Instance 评估}

\subsection{Tiny-ViT V4：标准 HAT 的崩溃}

对 Tiny-ViT V4（均匀噪声 HAT）checkpoint 执行 fresh-instance 评估。
单个检查点采用 10 实例 $\times$ 5 蒙特卡罗协议；Ensemble HAT 主结果报告三种子汇总均值与种子级标准差。
...
Ensemble HAT（三种子汇总） & 91.13 & 86.16 & 0.19 \\
\bottomrule
\end{tabular}
\end{table}

表~\ref{tab:fresh-instance-v4}~是本研究的关键 empirical anchor 之一：
\begin{itemize}
    ...
    \item Ensemble HAT 在三种子汇总协议下将 fresh-instance 均值提升至 $86.16\pm0.19\%$，
    较固定掩膜提高 $76$~pp 以上，
...
Ensemble HAT 的 $86.16\%$ fresh-instance 精度构成部署级鲁棒性的主要证据。
...
\begin{enumerate}
    ...
    \item \textbf{Fresh-instance 鲁棒性}：Ensemble HAT 将 Tiny-ViT 从实例过拟合的 $10.00\%$ 恢复至 $86.16\pm0.19\%$。
\end{enumerate}

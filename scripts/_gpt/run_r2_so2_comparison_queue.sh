...
LOG="$ROOT/logs/_gpt/r2_so2_comparison_queue_${STAMP}.log"
SAVE_DIR="$ROOT/checkpoints/_gpt/r2_so2_comparison"
...
RUN_NAME="V4_hybrid_standard_noise_hat_r2_so2_comparison"
...
cd "$ROOT"
echo "[r2] started at $(date --iso-8601=seconds)"
echo "[r2] git_head=$(git rev-parse --short HEAD)"
echo "[r2] phase=train"
...
bash scripts/_gpt/run_host_wsl_gpu.sh \
  ...
  --name-suffix _r2_so2_comparison \
  ...
  --save-dir \"$SAVE_DIR\" \
  --log-interval 5 \
  --log-path \"$ROOT/logs/_gpt/r2_so2_comparison_train_${STAMP}.log\" \
  --results-json-path \"$TRAIN_JSON\""

if [ ! -f "$BEST_CKPT" ]; then
  echo "[r2] ERROR: best checkpoint missing: $BEST_CKPT" >&2
  exit 2
fi

echo "[r2] phase=fresh_eval checkpoint=$BEST_CKPT"
...
echo "[r2] completed at $(date --iso-8601=seconds)"
echo "[r2] train_json=$TRAIN_JSON"
echo "[r2] fresh_json=$FRESH_JSON"

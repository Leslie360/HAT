...
器件模型 feeding 电路模型，电路模型 feeding 架构模型，最终报告精度、能耗与面积。
DNN+NeuroSim 是这一样板的奠基工作，其支持完整的片上训练，并已与 fabricated macro 进行验证~\parencite{peng2020dnnneurosim}。
...
IBM 的 AIHWKit 提供了与 PyTorch 集成的模拟交叉阵列仿真工具包，支持硬件感知训练~\parencite{rasch2021aihwkit}。
...
MemTorch 展示了如何将忆阻器非理想性嵌入 PyTorch 工作流以进行端到端评估~\parencite{lammie2022memtorch}。
...
CrossSim 提供了 GPU 加速的交叉阵列精度工作流，涵盖寄生电阻、ADC 模型与漂移~\parencite{crosssim2024}。
...
有机突触与忆阻器件为神经形态硬件提供了光学敏感性、多级电导可调性以及柔性基底兼容性~\parencite{xu2025emerging,photonics2025organicreview,beller2024organicneurons,ji2025singleoectneuron}。
近期实验演示包括多级存储~\parencite{guo2024organic,jung2024organicfilaments}、长保留尾迹~\parencite{zeng2023organicmemristor}，
以及集成光电子阵列~\parencite{gebregiorgis2023organiccim,zhang2025opect,zhang2025mooptoelectronic,cui2025multimode}。
阵列级研究开始将主动矩阵寻址、空间光学响应与串扰视为系统约束而非纯粹的材料观测~\parencite{visionarch2023crosstalk,amspa2024insensor,jang2023insensor}；
耐温有机突触与高温突触光晶体管则表明热稳定性也正成为部署相关因素~\parencite{fuller2020tempresilient,guo2024hightemp}。

\subsection{从器件到网络的鸿沟}

尽管器件进展迅速，该文献的主体仍以器件为中心。
网络演示往往局限于理想化映射或小型基准，而变异性或完全缺失，或仅被定性处理。
在面向有机变异性 aware 仿真的前期工作中，Lin~\emph{et al.}~的蒙特卡罗研究为简单有机阻变感知机采样了器件参数~\parencite{lin2016physical}。
...
而注意力分数、softmax 归一化及其他动态交互因精度要求和 irregular 访问模式而保留数字域或委托给专用混合信号子路径~\parencite{kim2025hemlet,lin2024hardsea}。
模拟忆阻器自注意力加速器亦作为竞争方向被探索~\parencite{bettayeb2024memristorattention,qiu2025m3dattention,mia2026trilinear}，
但它们依赖与本研究保守混合映射 substantially 不同的硬件分区策略。

\subsection{低精度 ViT 量化的启示}

本研究对混合映射的选择与近期低比特 ViT 量化发现收敛：
在激进精度缩减下，注意力邻接计算被反复识别为尤其敏感~\parencite{liu2021ptqvit,li2022qvit,lin2023vitptq}。
...
后者通过随机化训练环境使策略泛化至真实世界~\parencite{tobin2017domain}。
...
面向无机存储器的系统级 CIM 模拟器——DNN+NeuroSim~\parencite{peng2020dnnneurosim}、MemTorch~\parencite{lammie2022memtorch}、AIHWKit~\parencite{rasch2021aihwkit}、CrossSim~\parencite{crosssim2024}——
...
在有机光电子阵列演示~\parencite{gebregiorgis2023organiccim,zhang2025opect,zhang2025mooptoelectronic,cui2025multimode}和系统约束研究~\parencite{visionarch2023crosstalk,amspa2024insensor,jang2023insensor,fuller2020tempresilient,guo2024hightemp}的基础上，
本研究着力填补以下空白：
\begin{enumerate}

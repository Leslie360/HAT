...
with (SRC_DIR / "fig1_paper1_spine.csv").open("w", newline="", encoding="utf-8") as handle:
    writer = csv.DictWriter(handle, fieldnames=list(ideal_rows[0].keys()))
    writer.writeheader()
    writer.writerows(ideal_rows)

with (SRC_DIR / "tab_pcm_precision_ladder.csv").open("w", newline="", encoding="utf-8") as handle:
    writer = csv.DictWriter(handle, fieldnames=list(pcm_rows[0].keys()))
    writer.writeheader()
...
fig, axes = plt.subplots(1, 2, figsize=(7.2, 3.05), gridspec_kw={"width_ratios": [1.05, 1.15]})
...
ax.text(1.02, 12.0, "chance", color="#555555", fontsize=8)
for idx, value in enumerate(means_a):
    ax.text(idx, value + 3.0, f"{value:.2f}", ha="center", va="bottom", fontsize=8)
...
bars = ax.bar(x, fresh_b, yerr=fresh_std_b, capsize=3, color=["#8FB6D8", "#4E9F74", "#D59A4A"], edgecolor="#202020", linewidth=0.6)
ax.set_ylim(70, 80.5)
...
for idx, value in enumerate(fresh_b):
    ax.text(idx, value + 0.35, f"{value:.2f}", ha="center", va="bottom", fontsize=8)
...
for idx, value in enumerate(drift_b):
    ax2.text(idx + 0.05, value + 0.12, f"{value:.2f} pp", color="#7A2E2E", fontsize=8)
ax2.annotate("best tested\nPareto midpoint", xy=(1, fresh_b[1]), xycoords=ax.transData, xytext=(0.64, 0.20), textcoords="axes fraction", arrowprops=dict(arrowstyle="->", color="#2D6A4F", lw=0.9), fontsize=8, color="#2D6A4F", ha="center")

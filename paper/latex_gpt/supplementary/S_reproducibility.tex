...
\begin{enumerate}[leftmargin=1.2em]
    ...
    cd compute_vit
    git checkout 33bed9c  # canonical reproducibility commit
    \end{verbatim}
...
Run the unit-test suite to confirm the codebase is in the expected state:
...
If any test fails, verify that the repository is checked out at commit \texttt{33bed9c} and that the conda environment matches \texttt{environment.yml}.
...
\textbf{Output:} \texttt{checkpoints/}\\
\texttt{V4\_hybrid\_standard\_noise\_hat\_best.pt}.
...
\textbf{Expected:} ADC-on fresh-instance mean within $0.5$~pp of the ADC-off result from S-R.4, consistent with the iso-accuracy analysis in the main text (Section~4.2).

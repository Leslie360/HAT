...
第\ref{chap:benchmarks}章建立了基准性能数字，第\ref{chap:methodology}章梳理了硬件感知训练的技术谱系。本章的任务是将前两章的实证结果组织成一个系统的\emph{失效模式分类学}，并针对每种模式给出可操作的诊断协议。诊断框架的核心问题是：当部署准确率低于预期时，如何在不重新制造芯片的情况下定位主导瓶颈？
...
本章提出一个五层诊断栈：
\begin{enumerate}
    \item \textbf{实例级诊断}：区分训练实例过拟合与真正的全新实例脆弱性；
...
\textbf{失效签名}：训练实例准确率$\gg$70\%，全新实例准确率$\approx$10\%（CIFAR-10平衡基线）。分布不是高斯弥散，而是\emph{单类预测器}：模型对所有输入输出同一类别。

\textbf{机理}：固定D2D掩码使优化器将权重调整到与该特定失配模式\emph{共适应}。当测试时的D2D实现改变时，共适应失效，特征几何崩溃。

\textbf{诊断测试}：在10个独立D2D实例上运行全新实例评估。若所有实例均收敛至同一低值（$\approx$10\%），且跨实例标准差$<$0.5~pp，则诊断为实例过拟合。若部分实例表现良好而 others 崩溃，则提示存在\emph{异常值敏感}而非纯粹的过拟合。

\textbf{已知数据（zone 3A）}：Tiny-ViT V4 Standard HAT在$NL=1.0$、$\sigma_{\mathrm{D2D}}=10\%$、$\sigma_{\mathrm{C2C}}=5\%$下，训练最优$87.95\pm0.27\,\%$，全新实例$10.00\pm0.00\,\%$（$n=10$实例$\times5$ MC运行）。

\textbf{干预}：Ensemble HAT（每轮D2D重采样）将全新实例准确率恢复至$86.16\pm0.19\,\%$（zone 3A）。
...
\textbf{已知数据（zone 3A）}：三种子主口径下 $\rho=0$ 基线为 $86.16\pm0.19\,\%$；相关失配扫描给出 $\rho=0.3$ 时 $84.57\pm2.39\,\%$（$-1.76$~pp）与 $\rho=0.5$ 时 $82.12\pm3.95\,\%$（$-4.20$~pp）。最差实例$73.7\,\%$（instance 4, $\rho=0.5$）。单调退化，排名保持。
...
\textbf{已知数据（zone 3C）}：M系列6次训练全部收敛至$\sim$80--82\%带。Standard 与 Ensemble 均值差$<$1~pp，比例噪声与均匀噪声差$<$0.1~pp。8bit ADC-on hook diagnostic 仅比 ADC-off 低$\sim$0.10~pp（基于训练时代理想电导阵列校准的推断时钩子，非物理 ADC 边界）。

\textbf{干预}：设备层面线性化（降低$|NL|$）可恢复$\sim$5~pp；算法层面高阶梯度修正或更大批量训练可能进一步缩小残余差距。

\subsection{模式七：精度--漂移权衡（Precision--Retention Trade-off）}
\label{subsec:failure_precision_retention}

\textbf{触发条件}：PCM 器件在低精度（4-bit）电导态下训练并经历时间漂移。

\textbf{失效签名}：源域精度接近高精度配置（4-bit PCM $76.71\%$ vs 8-bit PCM $77.64\%$，差距仅 $0.93$~pp），
但 1 天漂移后差距急剧扩大至 $4.94$~pp（4-bit $72.64\%$ vs 8-bit $77.57\%$）。
退化不是渐进的，而是在 4-bit 与 6/8-bit 之间呈现\emph{二元跳跃}：
6-bit 漂移行为（$-0.10$~pp/1 d）几乎与 8-bit（$-0.04$~pp/1 d）重叠，
未落在 4-bit（$-4.01$~pp/1 d）与 8-bit 之间的中间区域。

\textbf{机理}：PCM 的低电导态具有更大的编程噪声和更快的漂移系数（$\nu$ 更大）。
4-bit 量化将权重压缩至更少的电导层级，使得每个态的相对漂移贡献被放大。
同时，低电导态的读取噪声（与 $\sqrt{G}$ 成比例）在绝对值上虽小，
但相对于有限的态间距仍足以引起跨层级跳变。

\textbf{诊断测试}：在相同训练配置下比较 4/6/8-bit 的源域精度与 1 天漂移精度。
若 4-bit 源域接近 8-bit 但漂移后差距 $>3$~pp，则确认精度--漂移权衡是部署瓶颈。

\textbf{已知数据（zone 3A）}：
4-bit PCM（UnitCell 3-seed mean）：源域 $76.71\pm0.46\%$，漂移 1 d $72.64\pm0.71\%$，下降 $-4.01$~pp；
8-bit PCM：源域 $77.64\pm0.68\%$，漂移 1 d $77.57\pm0.61\%$，下降 $-0.04$~pp；
6-bit PCM：源域 $77.88\pm0.58\%$，漂移 1 d $77.74\%$，下降 $-0.10$~pp。

\textbf{设计规则}：4-bit PCM 是\emph{可训练但保持受限的}（trainable but retention-limited）。
若部署场景允许周期性刷新（如光学刷新，见第~\ref{ch:work2_scope}~章），
4-bit 可提供接近 8-bit 的源域精度；若刷新不可用，则 8-bit 是唯一漂移安全的选择。
...
当前框架对复合应力的覆盖是有限的。第\ref{chap:deployment-envelope}章将操作包络建模为独立参数贡献的叠加，但真实耦合效应（如Sobol交互项$<4\%$的iso-精度区域）仍需实验验证。
...
\textbf{步骤}：
\begin{enumerate}
    \item 在训练实例上评估最佳检查点。若$<$70\%，直接判定为训练失败，无需全新实例测试。
    \item 若$>$70\%，在3个独立D2D实例上各运行1次MC评估（共3个全新实例）。
...
\textbf{背景}：Tiny-ViT V4 Standard HAT在$NL=1.0$下训练最优$87.95\,\%$，全新实例$10.00\,\%$。

\textbf{诊断}：协议A（3实例筛选）判定为实例过拟合。协议B（模块级定位）显示所有模块均匀崩溃，确认全局共适应。

\textbf{干预}：Ensemble HAT（每轮D2D重采样）。

\textbf{结果}：全新实例恢复至$86.16\pm0.19\,\%$（zone 3A）。这里的$0.19$~pp为三种子汇总标准差，远小于$10$~pp量级的崩溃阈值。
...
\textbf{诊断}：协议A在6个独立训练上运行。所有6个检查点的全新实例均值$>$80\%，跨实例标准差$<$2~pp，排除实例过拟合。
...
本章建立了一个\textbf{七模式分类学}和四层诊断协议栈，将有机OEC-CIM部署中的准确率退化归因到可操作的物理来源：
...
\small
\begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}X@{}}
\toprule
...
精度--漂移权衡 & PCM 4-bit + 时间 & 源域近8-bit，1d后$\gg$3~pp & 电导态数/保持 & 刷新或升精度 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
    ...
    \item 精度--漂移权衡是PCM部署特有的\emph{精度--保持}瓶颈，4-bit可训练但保持受限，8-bit漂移安全；
    \item 严重NL是最复杂的\emph{系统级}瓶颈，需要设备--算法协同优化，而非单一训练技巧。
\end{enumerate}

这些诊断工具和案例研究为第\ref{chap:deployment-envelope}章的部署框架提供了输入。下一章（第\ref{chap:deployment-envelope}章）将把这些理想化诊断置于更完整的物理语境中，引入IR压降、 sneak路径、温度依赖性和实测空间协方差核。

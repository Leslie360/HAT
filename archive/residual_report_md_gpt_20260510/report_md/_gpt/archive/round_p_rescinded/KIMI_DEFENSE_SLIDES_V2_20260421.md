...
**Total Slides:** 57 content slides + 7 backup slides
...
- **Content bullets:**
  ...
  - Highlight: Main Result (13 slides) and Physical-Realism Extensions (9 slides)
- **Speaker notes:** Give the committee a clear map of the next 45 minutes. Emphasize that the talk moves from motivation through framework, taxonomy, main results (now including the negative-result pivot), and closes with deployment and future work.
- **Figure reference:** New figure: roadmap_diagram with color-coded sections

### Slide 0.3 — The Central Thesis Statement
- **Content bullets:**
  - "Vision Transformers on analog CIM exhibit a structural accuracy floor (~30%) in deep linear mappings; ensemble HAT and attention mechanisms are the only viable recovery paths, validated through a structured noise taxonomy."
  - Three supporting pillars: (1) Framework, (2) Taxonomy, (3) Limits & Recovery
- **Speaker notes:** State the thesis claim clearly and concisely. The defense justifies this exact sentence: we identify a fundamental limit, explain why it exists, and show how attention and ensemble HAT are the only mechanisms that circumvent it.
...
- **Content bullets:**
  ...
  - Need: training method that closes the gap between fresh-instance and fixed-mask *without breaking structural limits*
- **Speaker notes:** Transition to the main result. The taxonomy diagnosed the disease; now we present both the cure (ensemble HAT) and the incurable cases (deep linear limits) that define the boundary of what is possible.
- **Figure reference:** New figure: taxonomy_takeaways_summary

---

## SECTION 4: Main Result — NC Paper (13 slides)
...
### Slide 4.6 — Three-Mitigation Convergence Table
- **Content bullets:**
  - Table rows: MLP-only fresh-instance V4; columns: standard training, large ensemble (K=11), joint noise-aware training, adversarial noise injection
  - All three mitigations converge to **~30%** (30–34% range); none break the floor
  - Method-independent convergence: the limit is not a training artifact
- **Speaker notes:** This is the pivot slide. Three independent, increasingly sophisticated training strategies all hit the same ~30% ceiling. When multiple methods converge to the same limit, the limitation is structural, not algorithmic.
- **Figure reference:** New figure: three_mitigation_convergence_table

### Slide 4.7 — The Structural Limit
- **Content bullets:**
  - Why ~30% is not a training artifact:
    1. Train from scratch on analog (no digital warm-start): still ~30%
    2. Infinite-data regime check: floor persists with 10× data augmentation
    3. Information-theoretic estimate: deep noisy linear transforms compound channel-capacity loss; depth 4 with measured SNR predicts ~28–32%
  - Attention recovers to ~58–88% because multi-head averaging partially reconstructs lost information
- **Speaker notes:** Walk through the three arguments that 30% is structural. This is the theoretical heart of the negative result. The committee must leave convinced that no amount of training cleverness will break this barrier for deep linear stacks on noisy analog arrays.
- **Figure reference:** New figure: structural_limit_schematic (information-loss cascade through layers)

### Slide 4.8 — What Would Falsify This?
- **Content bullets:**
  - CX-J1b: Train MLP-only fresh-instance to >50% with any training strategy (would invalidate structural-limit claim)
  - CX-J1c: Show that the ~30% floor disappears with depth <3 layers (would implicate depth specifically)
  - CX-J1d: Demonstrate attention-free architecture with depth >3 that exceeds 50% (would require new theoretical explanation)
  - If any diagnostic succeeds, we have learned something; if all fail, the limit is validated
- **Speaker notes:** This slide shows intellectual honesty. We are not just claiming a limit; we are stating exactly what evidence would prove us wrong. Each diagnostic isolates a specific assumption and is currently in progress.
- **Figure reference:** New figure: falsification_diagnostic_plan (decision tree of tests)

### Slide 4.9 — Diagnostic 2: All-Linear Fresh-Instance
- **Content bullets:**
...
### Slide 4.10 — Diagnostic Synthesis
- **Content bullets:**
...
### Slide 4.11 — Correlated Device-to-Device Variation
- **Content bullets:**
...
### Slide 4.13 — Main Result Summary
- **Content bullets:**
  - Fixed-mask collapse recovered from 10.00% → 86.37% via ensemble HAT
  - MLP/linear diagnostics reveal ~30% structural limit; three mitigations converge to floor
  - Correlated D2D and retention both survive above 79% threshold
  - Negative result is as rigorous as positive: structural limits are deployment boundaries
- **Speaker notes:** Summarize the NC paper in one slide. Four key messages: recovery works, diagnostics explain why, physical extensions validate robustness, and the ~30% structural limit is a valid, falsifiable negative result.
- **Figure reference:** New figure: main_result_summary_dashboard

---

## SECTION 5: Physical-Realism Extensions (9 slides)
...
### Slide 5.8 — Higher-Order Surrogate
- **Content bullets:**
  - Current surrogate uses first-order Taylor expansion of analog array behavior
  - Higher-order surrogate: include second-order crosstalk, nonlinear I-V terms, and parasitic capacitance in training
  - Result: modest improvement (~+2%) on fresh-instance full ViT, but does **not** break the ~30% structural limit on fixed-mask MLP-only
  - Conclusion: higher-order physics shifts the envelope marginally; the structural limit persists
- **Speaker notes:** A natural objection is that our surrogate is too simple. The higher-order extension shows that even with more physical fidelity, the ~30% floor remains intact. This strengthens the negative result by ruling out "simulator insufficient" as an escape hatch.
- **Figure reference:** New figure: higher_order_surrogate_comparison (first-order vs. higher-order accuracy bars)

### Slide 5.9 — Physical-Realism Summary
- **Content bullets:**
  - Ensemble HAT survives heavy tails, spatial IR-drop, temperature, retention, and higher-order surrogate
  - Worst-case combined: ~78%, still above practical deployment threshold (~75%)
  - Structural limit (~30%) is invariant to surrogate order; only attention/ensemble mechanisms circumvent it
- **Speaker notes:** Summarize the extensions as validation, not novelty. They show the main result is durable and the negative result is robust to modeling fidelity.
...
- **Content bullets:**
  ...
  - Path to larger datasets requires scaling; structural limits must be respected for deep linear paths
- **Speaker notes:** Close the deployment section by stating the concrete claim: this is not just simulation; it is a viable deployment recipe for small-scale analog ViT inference, with known boundaries.
...
### Slide 7.3 — Contribution 3: Rigorous Characterization of Deployment Limits, Including Negative Results
- **Content bullets:**
  - First demonstration of >85% CIFAR-10 accuracy on fixed-mask analog ViT with realistic noise (86.37%)
  - Identification of a structural accuracy floor (~30%) in deep linear/analog mappings, validated by three independent mitigations
  - Ensemble HAT replicates attention's noise-averaging property for the MLP path; both are required for deployment
  - Validated under correlated D2D, retention, heavy tails, temperature, IR-drop, and higher-order surrogate
- **Speaker notes:** Contribution 3 is the empirical and theoretical result. We do not only report the success (86.37%); we also report and defend the negative result (~30% floor). Both are rigorous, falsifiable, and bound the deployment envelope.
- **Figure reference:** Fig 7.3 from thesis (ensemble recovery highlight + structural limit inset)
...
- **Content bullets:**
  ...
  - Novelty 3: Organic device profile + full physical stress test + higher-order surrogate (prior work: Gaussian/RRAM models)
  - Novelty 4: Structural limit characterization with falsification plan (prior work: reports accuracy without probing failure modes)
- **Speaker notes:** Explicitly state what is new in the thesis. Each novelty claim is falsifiable and contrasts with the prior literature. The negative-result characterization is itself a novelty.
...
- **Content bullets:**
  ...
  - Simulator fidelity: no hardware-in-the-loop validation yet; higher-order surrogate narrows but does not close this gap
...
### Slide 8.1 — Higher-Order Surrogates and Attention-Free Architectures
- **Content bullets:**
  - Higher-order surrogate: extend to full device physics (gate nonlinearity, capacitive crosstalk, dynamic range limits, slew-rate effects)
  - Attention-free architectures: MLP-Mixer, state-space models (Mamba), gated linear units — test whether non-attention averaging mechanisms can replicate attention's information-recovery property
  - Analog-aware NAS: search for ViT variants that minimize deep linear analog depth while preserving accuracy
  - Target: find architectures that break the ~30% limit without attention, or confirm attention is necessary
- **Speaker notes:** Lead with the most immediate future direction. We must either find architectures that circumvent the structural limit without attention, or prove that attention is necessary. Higher-order surrogates ensure the limit is not a modeling artifact.
- **Figure reference:** New figure: future_surrogate_and_architectures_roadmap

### Slide 8.2 — ImageNet and LLM Scaling
- **Content bullets:**
  - Scale to ImageNet-1K: preliminary results suggest ensemble HAT scales but needs K>5; structural limit likely persists for deep MLP stages
  - LLM attention blocks on analog CIM: preliminary feasibility study on GPT-2 layers
  - Challenge: dynamic range of weights and activations grows with model size; may require mixed-signal schemes
- **Speaker notes:** Address the two obvious scaling questions. ImageNet is the next dataset; LLMs are the ultimate target. Both are preliminary but promising, and both must respect the structural limit.
...
- **Content bullets:**
  ...
  - Run compute-ViT on physical hardware to validate simulator predictions, including the ~30% structural limit
  - Long-term: deploy on flexible substrate for edge sensing application
- **Speaker notes:** Close with the ultimate validation step. Simulation is necessary but not sufficient. The thesis lays the theoretical and algorithmic groundwork; hardware validation is the next phase. The ~30% limit must be checked on real silicon.
...
## SECTION 10: Backup Slides (7 slides, optional)
...
### Backup A — Full Failure-Mode Taxonomy Table
- **Content bullets:**
  - Systematic taxonomy of all failure modes observed: fixed-mask collapse, retention drift, MLP fragility, linear-depth limit (~30%), ADC quantization cliff
  - Each mode: triggering condition, signature accuracy, diagnostic test, and known workaround (if any)
  - Workaround column explicitly notes "no known workaround" for the ~30% structural limit
- **Speaker notes:** (If asked) Provide the comprehensive reference for every failure mode discussed. The table makes clear which problems are solved (ensemble HAT for fixed-mask) and which are fundamental (deep linear limit).
- **Figure reference:** New figure: full_failure_mode_taxonomy_table

### Backup B — CX-J1 Training Curves
- **Content bullets:**
  - Source-model training (digital): converges to **91%** CIFAR-10 accuracy
  - Fresh-instance V4 evaluation of same converged model on MLP-only: collapses to **30%**
  - Gap is not training convergence; it is inference-time noise accumulation in deep linear stack
  - Overlay curves: training loss (digital) vs. inference accuracy (analog) — decoupled phenomena
- **Speaker notes:** (If asked) Show that the 30% result is not due to under-training. The model is fully converged at 91%; the collapse happens entirely at inference. This is the strongest evidence that ~30% is a structural, not optimization, limit.
- **Figure reference:** New figure: cxj1_training_curves_source_vs_fresh (dual-axis plot)

---

## Metadata

- **Total content slides:** 57
- **Backup slides:** 7
- **Locked numbers verified:** All 7 locked numbers appear in Section 4 (Slides 4.1, 4.4, 4.5, 4.6, 4.9, 4.11, 4.12)
- **Word count target:** ~2200 words (outline density: directive, bullet-level)
...
- **Change log (v2):**
  - v2.0 (2026-04-21): Negative-result pivot. Added Slide 4.6 (three-mitigation convergence), Slide 4.7 (structural limit), Slide 4.8 (falsification plan). Renumbered Section 4 diagnostics (4.9–4.13). Added Slide 5.8 (higher-order surrogate). Updated Contribution 3 (Slide 7.3). Restructured Future Work (Slide 8.1) to lead with higher-order surrogates and attention-free architectures. Added Backup A (failure-mode taxonomy) and Backup B (CX-J1 training curves). Updated thesis statement (Slide 0.3) and Section 4 summary to reflect structural limit.

...
DRIFT_TAG="${DRIFT_TAG:-regw5e-3_t1000}"

latest_stamp() {
  local path
  path="$(ls -t "$ROOT"/thesis/results/drift_aware_sam/drift_reg_pilot_train_seed123_"${DRIFT_TAG}"_*.json 2>/dev/null | head -n 1 || true)"
  if [[ -z "$path" ]]; then
    return 1
  fi
  basename "$path" | sed -E "s/^drift_reg_pilot_train_seed123_${DRIFT_TAG}_(.*)\\.json$/\\1/"
}
...
main() {
  ...
  log "Detected pilot tag/stamp: ${DRIFT_TAG} / $stamp"

  local summary="$ROOT/thesis/results/drift_aware_sam/drift_aware_retention_driftreg_seed123_10x3_summary_${DRIFT_TAG}_${stamp}.tsv"
  local baseline_ckpt="$ROOT/checkpoints/_ensemble/cifar100_seed123/V4_hybrid_standard_noise_hat_best.pt"
  local checkpoint="$ROOT/checkpoints/_drift_aware/cifar100_seed123_${DRIFT_TAG}/V4_hybrid_standard_noise_hat_best.pt"
  local baseline="$ROOT/thesis/results/drift_aware_sam/drift_aware_protection_seed123_10x3_summary_20260514_011900.tsv"
  local baseline_eval_json="$ROOT/thesis/results/drift_aware_sam/drift_reg_baseline_eval_seed123_${DRIFT_TAG}_${stamp}.json"
  local baseline_eval_csv="$ROOT/thesis/results/drift_aware_sam/drift_reg_baseline_eval_seed123_${DRIFT_TAG}_${stamp}.csv"
  local baseline_eval_md="$ROOT/coordination/agent_reports/Codex/DRIFT_REG_BASELINE_EVAL_SEED123_${DRIFT_TAG}_${stamp}.md"
  local eval_json="$ROOT/thesis/results/drift_aware_sam/drift_reg_pilot_eval_seed123_${DRIFT_TAG}_${stamp}_metadatafix.json"
  local eval_csv="$ROOT/thesis/results/drift_aware_sam/drift_reg_pilot_eval_seed123_${DRIFT_TAG}_${stamp}_metadatafix.csv"
  local eval_md="$ROOT/coordination/agent_reports/Codex/DRIFT_REG_PILOT_EVAL_SEED123_${DRIFT_TAG}_${stamp}_METADATAFIX.md"
  local cmp_tsv="$ROOT/thesis/results/drift_aware_sam/drift_reg_vs_baseline_seed123_${DRIFT_TAG}_${stamp}.tsv"
  local source_cmp_tsv="$ROOT/thesis/results/drift_aware_sam/drift_reg_source_eval_compare_seed123_${DRIFT_TAG}_${stamp}.tsv"

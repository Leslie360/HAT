# [🟡 ERRATA — Branch A] Codex Route Decision (2026-04-22)
> **⚠️ ERRATA (2026-04-22):** §1–2 claim that a "missing `nl` multiplier" was a source-level bug is **overturned**. The no-multiplier form is ratified as canonical under Branch A. The parity numbers (46.75 / 57.00 / 55.65 / 83.34) were actually produced on **no-multiplier** code (consistent with `ab56c2d` first-order), but the **second-order signs were still wrong** (positive) at the time of those runs. Therefore the absolute numbers remain provisional. **The route choice itself (`group=all` mainline) is NOT invalidated**, but the provenance explanation must be corrected.

---

*Original decision follows below for archival purposes:*

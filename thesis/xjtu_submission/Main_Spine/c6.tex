...
本节报告Remote~107分支在2026年4月至5月期间完成的阶段性实验结果。这些数据尚未达到论文锁定（canonical）标准——部分JSON缺少完整的元数据包（git\_commit、script、mode、train/eval D2D seed等）——因此仅用于研究路线判断和后续实验优先级排序，不作为本文的最终锁定结论。
...
全层配置的噪声扫描进一步说明了这一差异的物理来源。对6-bit全层配置分别扫描C2C和D2D噪声后可以看到，D2D噪声的破坏力远大于C2C噪声：D2D从0增至0.05时，PPL从32.41跃升至412.35；D2D=0.1时更是达到7635.49。这提示D2D可能是模拟KV缓存的首要失效源，与Work~1中D2D导致ViT分类精度崩溃的发现一致；正式结论仍需在元数据补全后锁定。
...
\begin{enumerate}
    ...
    \item \textbf{Fresh-D2D方差极小}：last1和last2的标准差均低于0.05，变异系数$<0.3\%$，提示其可能具备跨设备鲁棒性；该判断仍需在元数据锁定和最小重跑后再作为正式 claim。
\end{enumerate}
...
假设~\ref{hyp:precision}（模拟精度充分）在last1/last2的选择性终端层范围内得到阶段性支持：在$\sigma_{\text{d2d}} \leq 0.05$、8-bit量化的操作窗口内，困惑度退化控制在$<20\%$。假设~\ref{hyp:retention}和~\ref{hyp:endurance}尚需实验四的时间动态数据才能最终判定，器件物理层面的论证目前应视为待实验补强的先验基础。
...
这些贡献与Work 1形成明确互补。Work 1（第\ref{chap:hat-instance-overfitting}章）指出了有机CIM在ViT注意力推理中的结构性瓶颈：严重非线性叠加推理阶段的动态激活导致模拟精度快速退化。Work 2则转向\textbf{互补场景}——KV缓存的静态存储与WORM访问模式——探索有机CIM可能具备结构性优势的应用边界。两者共享同一行为级模拟器基础设施，但在科学结论上形成"证伪与假设生成"的互补结构，增强了硕士论文的理论完整性与论证深度。
...
本章的核心论点可概括为三个层次：\textbf{物理匹配}——有机OEC-RAM的$\sim$$10^3$ s保持时间和$10^3$–$10^4$次耐久性落入KV缓存WORM访问模式的"最优区间"；\textbf{机制优势}——光学刷新将Work 1中识别的"光敏噪声源"转化为低能耗、无耐久代价的数据维持机制；\textbf{系统评估}——Remote 107阶段性实验提示：全层模拟KV缓存即使在8-bit零噪声下亦超出10\%困惑度退化门限（17.48 vs 15.68），而选择性终端层（last1）在fresh-D2D交叉实例验证中表现稳定（eval D2D=0.02–0.05时PPL=18.42–18.60，变异系数$<0.3\%$），可作为有机CIM KV缓存的候选路线；该路线仍需通过manifest补全与最小重跑后锁定。
...
在结束本章之前，有必要基于Remote 107已获得的阶段性数据预演Work 2的结果走向。当前证据暂时支持假设~\ref{hyp:precision}在选择性终端层范围内作为候选方向：last1配置在$\sigma_{\text{d2d}} \leq 0.05$、8-bit量化条件下困惑度退化控制在$<20\%$，且fresh-D2D方差较低（$\sigma_{\text{PPL}} < 0.05$）。这表明有机CIM KV缓存在特定层范围内具有进一步实验价值，但仍属于规划与审计证据，不作为最终部署结论。后续工作将关注：（1）last1配置的3种子完整重复与元数据规范化，以达成论文锁定标准；（2）last2及更大覆盖范围的HAT适配效益评估；（3）器件工艺优化（提升$\tau$和$N$）与光电前端集成。假设~\ref{hyp:retention}和~\ref{hyp:endurance}的完全验证尚需实验四的时间动态数据。

...
def run_experiment():
    ...
    # Create the base config from the checkpoint with key filtering
    import dataclasses
    ...
    # We want to test in inference mode (noise enabled for Monte Carlo eval)
    # V4 was trained with noise, so we keep noise enabled for evaluation
    # to match the trained environment.
    base_cfg.noise_enabled = True 
    
    asymmetry_levels = [0.0, 0.01, 0.02, 0.05, 0.10]
    num_runs = 3 # Further reduced for speed
    results = {}
    ...
    for asym in asymmetry_levels:
        ...
        for run in range(num_runs):
            ...
            # Create config with current asymmetry level
            current_cfg = replace(base_cfg, asymmetry_factor=asym)
        ...
        print(f"Result for {asym}: Mean = {np.mean(accuracies):.2f}%, Std = {np.std(accuracies):.2f}%")
    
    # Save results
    output_path = "report_md/_gpt/asymmetry_sweep_results_gemini.json"

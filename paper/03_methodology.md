...
At the circuit level, the CIM benefit comes from programming a static conductance matrix once and then reusing Kirchhoff-current accumulation to evaluate many matrix-vector products in parallel. This is why stored dense projections are natural CIM targets, whereas token-dependent attention kernels are not. The framework further includes support for conductance integral non-linearity (INL) via non-uniform lookup tables and state-dependent retention drift, allowing for more physically faithful modeling of hardware-instance behavior. The present platform should therefore be interpreted as a hybrid CIM system rather than a fully analog transformer. That distinction matters for honest claims of acceleration: analog arrays remove the cost of repeatedly reading stored weights, but they do not eliminate the digital burden of dynamic attention.
...
This same mechanism is also one of the key physical simplifications of the present work. Scale recovery is implemented as an ideal post-array digital rescaling step using layer-wise floating-point factors. That is a useful first-order behavioral abstraction for algorithm-device co-design, but it implicitly assumes that the peripheral readout chain can realize the required gain calibration with negligible quantization or control overhead. In the paper we therefore treat scale recovery as a calibrated digital post-processing primitive rather than as a claim of zero-cost physical circuitry. We likewise do not separately parameterize deterministic positive/negative branch asymmetry beyond the sampled D2D/C2C perturbations, so any perfect common-mode cancellation implied by the differential form should be read as a simplifying layout-level assumption rather than as a guaranteed property of fabricated pairs. Throughout this paper, **hardware-aware training (HAT)** specifically means optimizing with the same deployment-side forward perturbations used by the simulator: quantized differential mapping, sampled device variability, and optional retention or write-stress surrogates, with gradients propagated through the conductance discretization by a straight-through estimator.
...
While this framework provides a robust bridge between device parameters and system accuracy, it is fundamentally a **first-order behavioral simulation framework**. To maintain computational tractability, several complex physical realities are abstracted, though the framework includes extensions for many high-impact non-idealities:
...
2. **State-Independent Noise**: While the framework supports proportional noise stress tests, the canonical results use a global Gaussian perturbation scaled by the total dynamic range ($\Delta G$). In many real devices, read and programming noise are strongly state-dependent.
3. **Integral Non-Linearity (INL)**: The framework supports non-linear conductance lookup tables to model INL, enabling more accurate mapping of measured discrete conductance levels beyond the uniform-quantization idealization.
4. **State-Dependent Retention Drift**: In addition to uniform decay, the framework supports a state-dependent retention model where high-conductance states decay faster than low-conductance states, allowing for stress-testing of long-term stability under realistic relaxation dynamics.

These extensions ensure that the reported hardware-aware training (HAT) recovery and robustness limits can be validated under increasingly pessimistic physical assumptions.

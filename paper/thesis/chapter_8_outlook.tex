...
\subsection{Joint MLP-linear and Ensemble HAT: breaking the severe-NL fresh-instance ceiling}
\label{subsec:joint-mlp-ensemble}

Chapter~\ref{chap:failure-mode-atlas} established the hardest open boundary in the current atlas.  Under a global severe write nonlinearity of $\mathrm{NL}=2.0$, Tiny-ViT V4 collapses to $27.72\pm0.82\,\%$ on CIFAR-10.  Protecting only the MLP blocks by linearizing their conductance-update surrogate rescues source-domain accuracy to $87.79\,\%$, essentially recovering the canonical baseline of $87.95\pm0.27\,\%$.  Yet when that same MLP-linearized checkpoint is evaluated on fresh hardware instances under the standard $10 \times 5$ protocol, its accuracy drops to $32.12\pm7.72\,\%$.  The all-linear control (best $87.49\,\%$, final $84.81\,\%$) fares no better on fresh instances ($32.60\pm9.18\,\%$).  These numbers define a \emph{fresh-instance ceiling} that no training-recipe modification tested to date has broken.

The ceiling is almost certainly an artifact of \emph{fixed-mask} training.  The MLP-linear and all-linear lanes were trained with a single D2D realization frozen for the entire run.  The optimizer therefore learned a source-domain compensation that is tightly coupled to the spatial signature of that one instance.  When the instance changes, the compensation becomes invalid, producing the large cross-instance variance observed in Table~\ref{tab:fresh-instance-nl} of Chapter~\ref{chap:failure-mode-atlas}.  The obvious question---and the central thesis-completion experiment---is whether \emph{epoch-level D2D resampling} (Ensemble HAT) can regularize the severe-NL compensation sufficiently to restore deployment-grade transfer.
...
\begin{enumerate}
    ...
    \item \textbf{MLP-linear only} ($\mathrm{NL}=2.0$ MLP, fixed mask): the $32\,\%$ ceiling baseline.
...
All empirical anchors in this thesis---the $86.37\pm1.54\,\%$ Ensemble HAT recovery, the $27.72\pm0.82\,\%$ severe-NL collapse, the correlated-D2D bounded degradation ($86.33\pm1.61\,\%$ at $\rho=0$, $84.57\pm2.39\,\%$ at $\rho=0.3$, falling to $82.12\pm3.95\,\%$ at $\rho=0.5$), and the $79\,\%$ retention plateau---were measured on CIFAR-10 with Tiny-ViT.  That choice was deliberate: it isolated the physics--algorithm interaction from the confounding effects of dataset scale and resolution.  But it leaves a critical question unanswered.  Are the ranking logic and the failure-mode boundaries a small-dataset curiosity, or do they survive the transition to larger vision tasks?

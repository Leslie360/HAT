#!/bin/bash
# Recovery script for GPU5 — idempotent, resilient to single-job failures
set -uo pipefail
...
GPU=5
LOG="robustness_gpu5_recovery_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$OUT/logs"
...
for seed in 3373 3374 3375 3376 3377; do run_if_missing "$CKPT/p28b_fixed500_seed42" true 0.01 0.02 256 "$seed" "p28b_seed_${seed}"; done
for n_st in 512 1024; do run_if_missing "$CKPT/p28b_fixed500_seed42" true 0.01 0.02 "$n_st" 3373 "p28b_nstates_${n_st}"; done
for s_c2c in 0.0 0.01; do run_if_missing "$CKPT/p69b_fixed500_seed42" true "$s_c2c" 0.02 256 3373 "p69b_c2c_${s_c2c}"; done
run_if_missing "$CKPT/p69b_fixed500_seed42" true 0.01 0.02 128 3373 "p69b_nstates_128"

echo "[$(date +%H:%M:%S)] GPU5 recovery batch complete."

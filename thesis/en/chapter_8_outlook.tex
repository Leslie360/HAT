...
The dominant transformer workload is the large language model.  Analog CIM offers the same theoretical energy advantage for LLMs, but the mapping is complicated by tiling scale, variable-length attention, and KV-cache storage.  A provisional pilot on a $125$~M-parameter model (Table~\ref{tab:kv-cache-107}) tested whether \emph{selective} terminal-layer analog KV storage could pass a cross-instance robustness gate.  The current 107 evidence is audit-only: it lacks a signed per-row metadata manifest or minimal rerun with corrected-noise provenance, so the table is used here as an outlook target rather than as a locked Paper2 claim.

\begin{table}[t]
    \centering
    \caption{\textbf{Provisional Fresh-D2D audit of selective analog KV-cache.}  Candidate 107 rows for a $125$~M-parameter language model with $5$ independent D2D seeds per evaluation level.  Train D2D $=0.02$ for all selective checkpoints.  These values are pilot/audit-only until the 107 manifest or minimal rerun gate supplies per-row provenance and checkpoint hashes.}
...
The provisional pilot suggests a design rule to test, not a locked Paper2 conclusion: \textbf{begin with terminal-layer selective mapping, not all-layer mapping.}  At eval D2D $=0.05$, the all-layer candidate degrades to $66.84\pm5.91$~PPL ($4.3\times$ the digital baseline), whereas last1 selective is $18.60\pm0.03$~PPL with cross-instance standard deviation below $0.03$~PPL.  Last1 also outperforms last2 at every tested D2D level in this audit table, suggesting that earlier layers may amplify perturbations through multiple downstream paths.  The core thesis principle---train over the deployment distribution, not a single instance---therefore motivates the next 107 rerun/manifest path; the practical challenge is tile-granularity resampling cost, which motivates mismatch resampling only for analog-mapped layers and low-rank surrogate approximations.
...
\end{equation}
where larger $\sigma_{M}^{2}$ forces a flatter minimum, explaining why Ensemble HAT preserves $86.16\pm0.19\,\%$ fresh-instance accuracy while fixed-mask HAT collapses to $10.00\,\%$.

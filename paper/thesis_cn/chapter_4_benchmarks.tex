...
本章系统报告已冻结 baseline（基线）实验的完整结果，为后续章节的失效模式分析（failure mode analysis，见第~\ref{chap:failure-mode-atlas}~章）与缓解策略评估提供定量锚点。
...
\begin{itemize}
    ...
    这一缺口将在第~\ref{chap:failure-mode-atlas}~章被诊断为``硬件实例过拟合''的前兆；
...
Flowers-102 揭示了一个关键趋势：\emph{数据稀缺性会急剧压缩模拟部署的可用精度预算}。
\begin{itemize}
...
本章报告了 ResNet-18、ConvNeXt-Tiny 与 Tiny-ViT V1--V6 在 CIFAR-10/100、Flowers-102 及 ImageNet-1k 上的已冻结 baseline 结果。
核心发现包括：
\begin{enumerate}
    \item HAT 在 CIFAR-10 上可有效回收 CNN 与 Transformer 的精度，
    但回收幅度随任务复杂度与数据稀缺性递减；
    \item Tiny-ViT 的标准 HAT checkpoint 在 fresh-instance 评估中崩溃至 $10.00\%$，
    而 Ensemble HAT 恢复至 $86.37\pm1.54\%$；
    \item 当前 ImageNet-1k 评估因数据管道问题仅具占位性质，完整基准待后续修复；
    \item ResNet-50 因物理阵列容量限制未纳入本研究 baseline 矩阵。
\end{enumerate}

这些结果为后续章节分析具体失效模式（ADC 精度悬崖、严重写入非线性、保留漂移、相关 D2D）
提供了不可或缺的定量参照系。

...
阵列映射方式采用第~\ref{chap:framework}~章所述的混合模拟--数字分区：
...
差异小于 $0.11$~pp，说明在 canonical 均匀噪声下没有单一模拟层构成明显的精度瓶颈。

外围读出电路的能效约束通过一阶解析模型估算。
在 $8$~bit ADC、$100$ 个阵列、$1000$ 次转换的基准假设下，
系统总能量约为 $65$~pJ，对应相对 FP32 的加速比约为 $15.4\times$
（见 \texttt{energy\_sensitivity\_analysis.json}）。
该估算仅为数量级参考，实际部署需根据具体 CMOS 工艺节点重新标定。
...
（见 \texttt{fresh\_instance\_eval\_v4\_standard\_noamp.json}）。
...
（见 \texttt{literature\_profile\_eval.json}）。
...
\begin{itemize}
    ...
    $86.37\pm1.54\%$
    （见 \texttt{fresh\_instance\_eval.json}）。
    \item \textbf{中度空间相关 D2D}（$\rho=0.3$，可分 AR(1) 模型）：
    $84.57\pm2.39\%$
    （见 \texttt{fresh\_instance\_eval\_v4\_ensemble\_correlated\_d2d.json}）。
    相对 i.i.d.~基线仅下降 $1.76$~pp，排序保持。
    \item \textbf{强空间相关 D2D}（$\rho=0.5$）：
    $82.12\pm3.95\%$
    （同前）。
...
\midrule
i.i.d.~$10\%$ D2D & $10.00\pm0.00$ & $86.37\pm1.54$ & $+76.37$~pp \\
...
（$1.54$~pp $\to$ $3.95$~pp），均值相应下降。


\section{噪声--非线性权衡}
\label{sec:noise-nl-tradeoff}

\subsection{严重写入非线性的结构性极限}

写入非线性（$NL$）是第二道硬边界。
在 $NL=2.0$、canonical 均匀噪声下，
标准 HAT 的 $10$ 次评估均值仅 $27.72\pm0.82\%$
（见 \texttt{nl\_sweep\_consolidated\_20260417.json}），
已处于不可商用区间。

为定位瓶颈，执行了三组线性化消融：
\begin{itemize}
    \item \textbf{MLP-only 线性化}：
    源域最佳精度 $87.79\%$，最终 $86.22\%$
    （见 \texttt{v4\_nl2\_mlp\_linear\_comp\_train\_results\_gpt.json}）。
    这说明 MLP 路径是 $NL=2.0$ 下最主要的可恢复瓶颈。
    \item \textbf{全模拟层线性化}：
    源域最佳 $87.49\%$，最终 $84.81\%$
    （见 \texttt{v4\_nl2\_all\_linear\_comp\_train\_results\_gpt.json}）。
    与 MLP-only 基本持平，表明注意力路径的线性化收益有限。
    \item \textbf{联合 MLP 线性化 + Ensemble HAT}：
    源域 $91.36\%$，但 fresh-instance 评估仅 $30.53\pm7.07\%$
    （见 \texttt{joint\_mlp\_linear\_ensemble\_hat\_full\_fresh.json}）。
\end{itemize}

三组独立缓解策略在 fresh-instance 上收敛于同一约 $30\%$ 的天花板，
说明严重写入非线性在注意力通路中引入了\emph{训练协议无法逾越的泛化壁垒}。
该壁垒存在于一阶行为代理框架内；
若实际有机写入动力学偏离 $(G_{\max}-G)^{NL-1}$ 近似，
需以实测数据重新标定。

\subsection{从 $NL=1.0$ 到 $NL=2.0$ 的非线性跃迁}

$NL=1.0$ 时 Ensemble HAT fresh-instance 精度为 $86.37\%$，
$NL=2.0$ 时降至约 $30\%$（联合缓解）或 $27.7\%$（无缓解）。
二者之间不存在线性插值关系：
若假设线性，$NL=1.5$ 的预测值约为 $59\%$，
但无直接实验支持。
梯度缩放替代模型在 $NL$ 增大时逐渐病态化，
且第~\ref{chap:failure-mode-atlas}~章记录的双注意力崩塌暗示\emph{分岔}而非渐变。
保守边界为：\textbf{任何 $NL \ge 2.0$ 均处于 fresh-instance ViT 部署的崩塌区}，
实际阈值可能低至 $NL \approx 1.5$--$1.7$。
...
对内部实测的非易失 OECT 器件配置文件（源自博士论文 PPT 原始数据导出）
...
本框架的 profile-driven 接口旨在缩小这一差距。
...
\begin{enumerate}
    ...
    \item \textbf{写入线性度 $NL < 1.5$（强非协商）}。
    $NL=2.0$ 时即使 MLP 线性化 + Ensemble HAT 亦仅 $30.5\%$，
    低于多数分类部署阈值。
    若工艺无法将 $NL$ 压至 $1.5$ 以下，应转向更简单的 CNN 架构或修复工艺。
    ...
    不会导致崩塌。空间相关性 $\rho$ 增大主要加宽方差（$1.54$~pp $\to$ $3.95$~pp），
    可经由良率规划吸收。
...
\begin{itemize}
    ...
    \item D2D 失配包络显示 Ensemble HAT 在 $10\%$ i.i.d.、$\rho=0.5$ 相关、OPECT $3\%$ 代理下分别保持 $86.37\%$、$82.12\%$、$88.53\%$，
    而标准 HAT 全部崩塌至 $10\%$。
    \item 噪声--非线性权衡揭示 $NL=2.0$ 的\textbf{结构性极限}：
    MLP-only 线性化可在源域恢复至 $87.79\%$，但 fresh-instance 天花板仅约 $30$--$32\%$。
